...
class SpacesItemDecoration : RecyclerView.ItemDecoration {
    ...
    private var mRadixX: Int = 0
    private var mItemCountInLastLine: Int = 0
    private var mOldItemCount = -1

    private var mInnerAdapter: BaseQuickAdapter<*, *>? = null

    constructor(space: Int, spanCount: Int = 1, adapter: BaseQuickAdapter<*, *>? = null) {
        mSpace = space
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        val headerLayoutCount = mInnerAdapter?.headerLayoutCount ?: 0
        val params = view.layoutParams as RecyclerView.LayoutParams
        val position = params.viewLayoutPosition
        if (headerLayoutCount > 0 && position < 1) return
        ...
        if (params is GridLayoutManager.LayoutParams) {
            ...
            if ((position == 0 || mOldItemCount != sumCount) && mSpanCount > 1) {
                ...
                for (tempPosition in sumCount - mSpanCount until sumCount) {
                    spanIndex = (parent.layoutManager as GridLayoutManager).getSpanSizeLookup()
