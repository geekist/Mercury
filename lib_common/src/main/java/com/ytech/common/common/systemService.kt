...
val Context.mediaSessionManager
    @SuppressLint("ServiceCast") @TargetApi(Build.VERSION_CODES.LOLLIPOP)
...
fun Context.isActivityExist(clazz: Class<*>): Boolean {
    var isActivityExist = false
    activityManager?.let {
        val intent = Intent(this, clazz)
        val componentName = intent.resolveActivity(packageManager)
        if (componentName != null) {
            val runningTasks = it.getRunningTasks(15)
            for (taskInfo in runningTasks) {
                if (taskInfo.baseActivity == componentName) {
