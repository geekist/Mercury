...
fun Context.loadDimen(@DimenRes id: Int) = resources.getDimension(id)
...
fun Context.loadStringArr(@ArrayRes id: Int): Array<String> = resources.getStringArray(id)

fun Context.getToolbarHeight(): Int {
    val ta = obtainStyledAttributes(intArrayOf(android.R.attr.actionBarSize))
    val height = ta.getDimensionPixelOffset(0, 0)
    ta.recycle()
    return height
}
...
fun Context.getRawSize(unit: Int, size: Float) = TypedValue.applyDimension(unit, size, resources.displayMetrics)
...
fun Fragment.loadAsset(fileName: String, accessMode: Int = AssetManager.ACCESS_STREAMING): InputStream? =
    activity?.loadAsset(fileName, accessMode)
...
fun View.loadAsset(fileName: String, accessMode: Int = AssetManager.ACCESS_STREAMING): InputStream? =
    context.loadAsset(fileName, accessMode)
