...
inline fun <reified T : Activity> Context.startActivity(extras: Bundle? = null, newTask: Boolean = false) {
    val intent = createIntent<T>(extras)
    if (newTask) {
 ...
 * 应用是否安装
 */
fun Context.isPkgInstalled(pkgName: String): Boolean {
